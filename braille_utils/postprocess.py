...
class Line:
    ...
    def check_and_append(self, box, label):
        ...
        if abs(self.y + self.slip*(x-self.x) -y) < self.h*self.LINE_THR:
            new_char = LineChar(box, label)
            if len(self.chars) >= self.MIN_AVG_PERIOD:
                calc_chars = self.chars[-self.AVG_PERIOD:]
                best_pair = [1e99, None, None, None, None, ] # err, i,j,k,a,b
                for i in range(len(calc_chars)-2):
                    for j in range(i+2, len(calc_chars)):
                ...
                self.h = new_char.h
                self.slip = 0
            step = self.h * self.SPACE_TO_H
            assert len(self.chars) >= 1
            new_char.spaces_before = max(0, round(0.5 * ((new_char.refined_box[0] + new_char.refined_box[2]) - (self.chars[-1].refined_box[0] + self.chars[-1].refined_box[2])) / step) - 1)
            self.chars.append(new_char)
            return True
        return False
